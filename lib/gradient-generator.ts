/**
 * Générateur de gradients aléatoires avec couleurs vives
 */
...
// Palette de couleurs vives prédéfinies
...
/**
 * Génère un gradient aléatoire avec deux couleurs vives
 */
export function generateRandomGradient(): Gradient {
  // Sélectionner deux couleurs différentes aléatoirement
  ...
  // Sélectionner une direction aléatoire
  const direction =
    GRADIENT_DIRECTIONS[
      Math.floor(Math.random() * GRADIENT_DIRECTIONS.length)
    ];

  // Générer le CSS gradient
...
/**
 * Génère un gradient avec des couleurs spécifiques
 */

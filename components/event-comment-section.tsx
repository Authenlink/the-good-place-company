...
export function EventCommentSection({ eventId, onCommentAdded }: EventCommentSectionProps) {
  ...
  const handleToggleLike = useCallback(async (commentId: number, currentLikes: CommentLikes | undefined) => {
    ...
    try {
      const response = await fetch(`/api/events/${eventId}/comments/${commentId}/likes`, {
        method,
      });
  ...
  const handleShowLikes = useCallback(async (commentId: number) => {
    ...
    try {
      const response = await fetch(`/api/events/${eventId}/comments/${commentId}/likes?list=true`);
      if (response.ok) {
        const data = await response.json();
  ...
  }, [eventId, fetchComments, toast, onCommentAdded]);
